...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Thu Nov 20 11:15:54 ART 2014
  */
...
class CUP$parser$actions {
    ...
    public int searchLevel(String id){
        for(Ambiente nivel : tds.getPila()){
            if(nivel != tds.getPila().getLast()){
		  ...
		  Descriptor d = tds.search(i);
						if (d!=null){
							if (d.getClase().equals("descriptorArreglo")){
								if(nivel == 0)						
										RESULT = new VarLocation(i,d,ileft,iright,false,true);
									else
		  ...
		  Descriptor d = tds.search(i);
						if (d!=null){
							if (d.getClase().equals("descriptorSimple")){
									if(searchLevel(i) == 0){
										System.out.println("HAGO REFERENCIA A UNA VAR GLOBAL!");				
										RESULT = new VarLocation(i,d,ileft,iright,false,true);
									}else{
										System.out.println("HAGO REFERENCIA A UNA VAR LOCAL!");
										RESULT = new VarLocation(i,d,ileft,iright,false,false);
									}	
							}else{
								parser.report_fatal_error("Los tipos no concuerdan",ileft,iright);}				
...
params = p;
		...
		String i = (String)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
						...
						if (tds.top().get(i)==null)
								if(nivel == 1){
									System.out.println("HAY UNA VARIABLE GLOBAL");
									RESULT = new VarLocation(i,new DescriptorSimple(i,tipoVar),ileft,iright,true,true);
								}else{
									System.out.println("HAY UNA VARIABLE LOCAL");
		...
		RESULT = start_val;
		 ...
		 parser.t=tds.clone();tds.pop();
          ...
          case 0: // NT$0 ::= 
            {
              Object RESULT =null;
 tds.push(new Ambiente(nivel++)); parser.t=tds.clone();
